...
const extractFrontmatter = (content) => { const m = content.match(/^(---\n[\s\S]*?\n---\n?)/) ; return m ? m[1] : '' }
...
for (const pkg of catalog.packages) {
  ...
  if (pkg.adapters.includes('claude')) {
    ...
    const claudeFm = extractFrontmatter(canonical)
    const claudeBody = stripFrontmatter(canonical).trimEnd()
    fs.writeFileSync(path.join(claudeDestDir, 'SKILL.md'), [claudeFm + generatedHeader, '', claudeBody, ''].join('\n'))
  ...
  if (pkg.adapters.includes('cursor')) {
    ...
    const cursorFm = extractFrontmatter(canonical)
    const cursorBody = stripFrontmatter(canonical).trimEnd()
    fs.writeFileSync(path.join(destDir, 'SKILL.md'), [cursorFm + generatedHeader, '', cursorBody, ''].join('\n'))
