...
grep -q 'git diff --name-only -z' "$CODEX_WORKFLOW"
grep -q 'git ls-files --others --exclude-standard -z' "$CODEX_WORKFLOW"
grep -q 'git restore --source=HEAD --staged --worktree' "$CODEX_WORKFLOW"
grep -q 'Codex native collaboration' "$SKILL"
grep -q 'OpenAI Codex' "$SKILL"
grep -q 'allowed-tools:.*spawn_agent.*wait_agent' "$SKILL"
grep -q "Codex uses one \`worker\` fixer" "$SKILL"
grep -q "read-only \`default\`" "$SKILL"
...
grep -q 'preserve that exact URL as the artifact result' "$ARTIFACT"
